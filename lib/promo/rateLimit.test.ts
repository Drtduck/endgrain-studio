...
describe('clientIp', () => {
  it('верит x-real-ip: его проставляет сам Vercel поверх присланного', () => {
    expect(clientIp('203.0.113.7, 70.41.3.18, 150.172.238.178', '198.51.100.4')).toBe('198.51.100.4')
    expect(clientIp(null, '198.51.100.4')).toBe('198.51.100.4')
    expect(clientIp('  ', '198.51.100.4')).toBe('198.51.100.4')
  })

  it('без x-real-ip берёт правый адрес цепочки, а не присланный клиентом левый', () => {
    // Левый элемент подделывается заголовком, и на каждый запрос это был бы новый счётчик.
    expect(clientIp('203.0.113.7, 70.41.3.18, 150.172.238.178', null)).toBe('150.172.238.178')

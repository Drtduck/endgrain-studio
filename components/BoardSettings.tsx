...
export function BoardSettings() {
  ...
  return (
    <Card>
      ...
      <CardContent className="@container/board space-y-3">
        ...
        {/* Число колонок считаем по ширине карточки, а не окна: в сайдбаре 268px
            даже две колонки едва вмещают значение, а sm:grid-cols-3 включался
            от ширины окна и резал цифры. */}
        <div className="grid grid-cols-2 items-end gap-2 @min-[400px]/board:grid-cols-3">
          <NumberFieldMm id="board-width" testId="board-width" labelKey="board.width" valueMm={design.board.targetWidthMm} unit={unit} locale={locale} minMm={BOARD_MIN_MM} maxMm={BOARD_MAX_MM} onCommitMm={setBoardWidthMm} size="compact" suffix={unitSuffix} />
          <NumberFieldMm id="board-length" testId="board-length" labelKey="board.length" valueMm={design.board.targetLengthMm} unit={unit} locale={locale} minMm={BOARD_MIN_MM} maxMm={BOARD_MAX_MM} onCommitMm={setBoardLengthMm} size="compact" suffix={unitSuffix} />
          <NumberFieldMm id="board-thickness" testId="board-thickness" labelKey="board.thickness" valueMm={design.board.thicknessMm} unit={unit} locale={locale} minMm={THICKNESS_MIN_MM} maxMm={THICKNESS_MAX_MM} onCommitMm={setBoardThicknessMm} size="compact" suffix={unitSuffix} />
          <NumberFieldMm id="board-kerf" testId="board-kerf" labelKey="board.kerf" valueMm={design.kerfMm} unit={unit} locale={locale} minMm={0.1} maxMm={10} onCommitMm={setKerfMm} size="compact" suffix={unitSuffix} />
          <NumberFieldMm id="board-allowance" testId="board-allowance" labelKey="board.allowance" valueMm={design.planingAllowanceMm} unit={unit} locale={locale} minMm={0} onCommitMm={setPlaningAllowanceMm} size="compact" suffix={unitSuffix} />
          <NumberFieldMm id="board-planer" testId="board-planer" labelKey="board.planerWidth" valueMm={design.planerWidthMm} unit={unit} locale={locale} minMm={50} onCommitMm={setPlanerWidthMm} size="compact" suffix={unitSuffix} />
        </div>
